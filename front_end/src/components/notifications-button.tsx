...
export function NotificationsButton() {
  ...
  const notifBody = (
    <div className="max-h-[min(23.75rem,60svh)] overflow-y-auto scrollbar-thin">
      {loading ? (
  ...
  if (isMobile) {
    return (
      <Sheet open={open} onOpenChange={setOpen}>
        ...
        <SheetContent
          side="right"
          className="flex w-[min(22.5rem,calc(100vw-2rem))] flex-col gap-0 p-0"
        >
          <SheetTitle className="sr-only">{t("notifications.title")}</SheetTitle>
          {/* Header — pr-10 clears the Sheet's built-in close button */}
          ...
          <div className="flex-1 overflow-hidden">{notifBody}</div>
          {notifFooter}
        </SheetContent>
      </Sheet>
    );
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="end"
        sideOffset={8}
        className="w-[min(22.5rem,calc(100vw-1rem))] p-0 overflow-hidden"
      >
        {/* Header */}
