...
CONFIDENCE_SAMPLES = 8  # Увеличенное количество положительных образцов для подтверждения
...
def recognize_faces():
    ...
    print("[INFO] Запущено распознавание. Ожидаем обнаружения лица или максимум", SCAN_DURATION, "секунд")
    camera_running = True
    ...
    try:
        while time.time() - start_scan_time < SCAN_DURATION and not stop_camera.is_set():
                ...
                if largest_face_idx >= 0:
                    ...
                    if (x2 - x1) >= min_face_size and (y2 - y1) >= min_face_size:
                        ...
                        try:
                            ...
                            face_img = cv2.resize(face_img, (160, 160))  # Размер, который ожидает FaceNet
                            face_img = cv2.cvtColor(face_img, cv2.COLOR_BGR2RGB)  # Перевод в RGB
                            ...
                            if embedding is None:
                                print("[WARNING] Не удалось получить эмбеддинг для обнаруженного лица")
                                continue
                            ...
                            print(f"[DEBUG] Обнаружено лицо: размер={x2-x1}x{y2-y1}, best match: {user_id}, similarity: {similarity:.4f}")
                            ...
                            if similarity > HIGHER_SIMILARITY_THRESHOLD:
                                ...
                                if last_matched_user != user_id:
                                    ...
                                    consecutive_matches[user_id] = consecutive_matches.get(user_id, 0) + 1
                                ...
                                if consecutive_matches.get(user_id, 0) >= CONFIDENCE_SAMPLES:
                                    ...
                                    if real_username != "Unknown":
                                        ...
                                        print(f"[INFO] Лицо уверенно распознано: {real_username} с коэффициентом {similarity:.2f}")
                                ...
                                cv2.rectangle(frame, (x1, y1), (x2, y2), (255, 165, 0), 2)  # Оранжевая рамка для средней уверенности
                                real_username = USERNAME_MAPPING.get(user_id, "Unknown")
                                label = f"{real_username}? ({similarity:.2f})"
                                cv2.putText(frame, label, (x1, y1 - 10),
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 165, 0), 2)
                                print(f"[INFO] Возможное совпадение: {real_username} с коэффициентом {similarity:.2f}")
...
async def check_attendance_use_ai(background_tasks: BackgroundTasks):
    ...
    if camera_running:
        return JSONResponse(
            status_code=400,
            content={"error": "Распознавание уже запущено"}
        )
...
async def get_recognition_result():
    ...
    try:
        ...
        response = requests.get(f'http://localhost:8000/api/get_student_information/{username}')
